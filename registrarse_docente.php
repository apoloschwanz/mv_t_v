...
if( isset($_POST['okregistrarse']) )
{
	...
	if ( $utls->existe )
		...
		{
		$id_nuevo_dte = $utls->Agregar_Docente();
		if ( $utls->hay_error() )
			...
			{ 
				...
				$txt = 'Se ha registrado como docente del programa Mi voto mi elección. Para completar sus datos ingrese a http://'.$sitio->nombre.' . Su contraseña es: '.$nouvellemot ;
				$headers = "From: ".$sitio->mailremitente . "\r\n" ;
				mail($to,$subject,$txt,$headers);
				$txt = 'Se registro un nuevo docente, con mail: '.$_POST['newid']. ' y cuyo id es: '.$id_nuevo_dte;
				$txt = "/n".'Se le envió un mail con la contraseña ' ;
				mail('daniel.zabalet@gmail.com',' Nuevo Docente ',$txt,$headers ) ;
				mostrar_alta() ; 
			}
		}
}
...
{	//
	...
	$pagina = new paginai('Alta de Docente',$botones) ;
	$pagina->sinborde() ;
	//
	// Mensaje
	if ( ! empty( $mje ) )
	...
	$texto = '<p>Nombre Completo</p>
			  ...
			  <td>
				...
				<font color="orangered" size="+1"><tt><b>*</b></tt></font>' ;
	$pagina->insertarCuerpo($texto);			
	$texto = '<p>Teléfono de Contacto</p>
			  </td>
			  <td>
