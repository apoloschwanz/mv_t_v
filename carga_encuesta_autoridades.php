<?php
	...
	//
	// Instancia Clases 
	$Rel1 = new Detalle_Observaciones_Encuesta() ; 
	//
	// Varibales de accion
	//
	...
	$okseleccion = $Rel1->obtiene_prefijo_campo().'_okSelected'; 
	//
	...
	elseif ( isset($_POST['nro_encuesta_ok']) )
		...
		$Enc->Set_Tipo_Encuesta_Cod('Autoridad_');
		...
		if ( $ok_capa == true  )
			{
				if ( $reg_capa['Programa_Nro'] == 1 )
					$Enc->Set_Conj_Preg_Cod('Aut__2016m');
				elseif ( $reg_capa['Programa_Nro'] == 5 )
					$Enc->Set_Conj_Preg_Cod('Aut__2016p');
				else
					$ok_capa = false ;
	...
	//
	// Confirma Núemro de Anexo - Viene de Link de Capacitaciones
	//
